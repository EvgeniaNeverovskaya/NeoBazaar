...
const PhotoUpload = ({ photos, setPhotos }) => {
  ...
  const handleFileChange = (event) => {
    ...
    if (invalidFiles.length > 0) {
      setAlert({ show: true, message: t("products.form.photo_alert_1") });
      return;
    }
    ...
    if (totalSize > 10000000) {
      setAlert({ show: true, message: t("products.form.photo_alert_2") });
      return;
    }

    if (photos.length + files.length > 3) {
      setAlert({ show: true, message: t("products.form.photo_alert_3") });
      return;
    }
  ...
  return (
    ...
    >
      <Box display="flex" justifyContent="space-between" width="100%">
        ...
        <Text textStyle="Ui/Body-medium" color="#7E88A4">
          {t("products.form.loaded")} {numberOfPhotosUploaded} {t("products.form.of")} 3
        </Text>
      </Box>
      {alert.show && (
      ...
      <Text  color="#7E88A4" textAlign="center">
